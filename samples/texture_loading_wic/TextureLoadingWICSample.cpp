...
{
  public:
    TextureLoadingWICSample::TextureLoadingWICSample()
        : SampleApplication("TextureLoadingWIC", 720, 720)
    {
    }
    ...
    virtual bool initialize()
    {
        ...
        HRESULT hr = WICTexImage2DFromFile(GL_TEXTURE_2D, 0, (widePath + L"\\fractal.tiff").c_str());
        if (FAILED(hr))
        {
            destroy();
            return false;
